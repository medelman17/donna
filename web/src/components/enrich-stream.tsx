...
export function EnrichStream({ login, onDone }: { login: string; onDone: () => void }) {
  ...
  useEffect(() => {
    ...
    const connectToStream = () => {
      const reader = new EventSource(`/api/enrich/${login}/stream`);
    ...
    const run = async () => {
      try {
        const res = await fetch(`/api/enrich/${login}`, { method: "POST" });
        if (cancelled) return;
        ...
        {status === "streaming" && (
          <span style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <button onClick={abort} style={{
              ...
              borderRadius: "var(--radius-DEFAULT)", padding: "2px 8px", fontSize: 11, fontWeight: 500, cursor: "pointer",
            }}>■ Stop</button>
          </span>
        )}
        {status === "done" && (
      ...
      <div ref={contentRef}>
              ...
              {(() => {
                ...
                while (i < blocks.length) {
                  ...
                  if (block.type === "text") {
                    ...
                    if (Component) {
                      rendered.push(
                        <motion.div key={block.id} initial={{ opacity: 0, y: 10, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }}
                    ...
                    while (i < blocks.length && blocks[i].type === "tool") { toolGroup.push(blocks[i]); i++; }
                    rendered.push(
                      <motion.div key={toolGroup[0].id} initial={{ opacity: 0, x: -4 }} animate={{ opacity: 1, x: 0 }}
                        transition={{ duration: 0.15, ease: "easeOut" }}
                        style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap", padding: "2px 0", fontSize: 11,
                          fontFamily: "var(--font-geist-mono), ui-monospace, monospace", color: "var(--color-fg-muted)" }}>
                        {toolGroup.map((t) => {
                          ...
                          return (
                            <span key={tb.id} style={{ display: "inline-flex", alignItems: "center", gap: 4 }}>
                              ...
                              <span style={{ padding: "1px 5px", borderRadius: 3, background: "var(--color-bg-2)", fontSize: 10.5, fontWeight: 500 }}>{tb.tool}</span>
                            </span>
                          );
                        })}
                      </motion.div>
        ...
        <AnimatePresence>
          {hasNewBelow && status === "streaming" && (
            <motion.button key="scroll-btn" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: 10 }} transition={{ duration: 0.2, ease: "easeOut" }}
              onClick={scrollToBottom} style={{
                ...
                color: "var(--color-accent)", fontSize: 11.5, fontWeight: 500, fontFamily: "inherit", cursor: "pointer", zIndex: 10,
              }}>↓ New content below</motion.button>
          )}
        </AnimatePresence>
      </div>
    </div>
